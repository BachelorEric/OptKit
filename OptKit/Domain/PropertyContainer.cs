...
namespace OptKit.Domain
{
    class PropertyContainer : IPropertyContainer
    {
        ...
        internal void CompileProperties()
        {
            ...
            {
                hierarchy[index].Properties.OrderBy(p => p.PropertyName).ForEach(p =>
                {
                    if (p.CompiledIndex == -1)
                    {
                        bool isCaculated = p is ICaculateProperty;
                        IProperty overrided;
                        if (_properties.TryGetValue(p.PropertyName, out overrided))
                        {
                            var x = overrided is ICaculateProperty;
                            if (x != isCaculated)
                                throw new AppException("[{0}]{1}属性[{2}]不能重写[{3}]{4}属性".FormatArgs(p.DeclareType.GetQualifiedName(), isCaculated ? "计算" : "非计算", p.PropertyName, overrided.DeclareType.GetQualifiedName(), x ? "计算" : "非计算"));
                            if (overrided.OwnerType == p.OwnerType)
                                throw new AppException("[{0}]已存在属性[{1}],不能重复注册,声明类型[{2}]".FormatArgs(overrided.OwnerType.GetQualifiedName(), overrided.PropertyName, p.DeclareType.GetQualifiedName()));
                            if (!isCaculated)
